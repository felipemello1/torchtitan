...
def _check_merge_compatibility(
    ...
    Args:
        ...
        context: Description for error messages (e.g., "WeightedScalar", "SumMetric").
...
class TMetricValue:
    ...
    def validate_placements(self, name: str) -> None:
        ...
        affect the final result. Each MetricValue subclass must validate that any Partial DTensor
        ...
        for attr_name in type(self).__annotations__:
            ...
            if reduce_ops:
                raise ValueError(
                    ...
                    f"before adding the summary, or use a MetricValue subclass that explicitly "
                    f"supports deferred reduction (e.g., WeightedScalar with Partial mean)."
                )
...
class MeanTMetric(TMetricValue):
    ...
    def value(self) -> Scalar:
        ...
        weight = _to_replicated_dtensor(weight)
        return _to_replicated_dtensor(sum_val / torch.clamp(weight, min=1e-8))

    def validate_placements(self, name: str) -> None:
        """Validates that DTensor placements are compatible with deferred reduction.

        For WeightedScalar with (sum, weight) representation:
        ...
        if invalid_sum_ops:
            raise ValueError(
                f"Summary '{name}' has a WeightedScalar with sum "
        ...
        if invalid_weight_ops:
            raise ValueError(
                f"Summary '{name}' has a WeightedScalar with weight "
    ...
    def merge_(self, other: Self) -> Self:
        ...
        Args:
            other: Another WeightedScalar to merge into this one.

        Returns:
            self, modified in-place.

        Raises:
            TypeError: If other is not a WeightedScalar, or if attempting to merge
    ...
    def __repr__(self) -> str:
        return f"WeightedScalar(sum={self._sum}, weight={self._weight})"
    ...
    @classmethod
    def __unflatten__(cls, leaves: list, context: None) -> "WeightedScalar":
...
class DerivedTMetric(TMetricValue):
    ...
    Example:
        ```
        mean_metric = WeightedScalar(mean=partial_mean, weight=1)
    ...
    _deps: list[MetricValue]

    def __init__(
        self,
        *,
        compute_fn: Callable[[list[Scalar]], Scalar],
        deps: list[MetricValue],
    ) -> None:
        ...
        Args:
            ...
            deps: A list of MetricValue objects whose values will be reduced and passed
                to compute_fn.
        """
    ...
    def __flatten__(self) -> tuple[list, tuple]:
        ...
        The deps (list of MetricValue) are the leaves; compute_fn is stored in context.
        """
...
def _to_replicated_dtensor(value: Tensor) -> Tensor:
    """Convert a Partial DTensor to Replicate. No-op for plain tensors.

    Ported from reference utils_spmd.py to_replicated_dtensor.
    """
    if not isinstance(value, DTensor):
        return value
    from torch.distributed.tensor.placement_types import Replicate

    target_placements = [Replicate()] * len(value.placements)
    if list(value.placements) == target_placements:
        return value
    return value.redistribute(value.device_mesh, target_placements)


def _replicate_list(values: list) -> list:
    """Replicate a list of values. Converts DTensors to local tensors."""
    result = []
    for v in values:
        if isinstance(v, DTensor):
            result.append(_to_replicated_dtensor(v).to_local())
...
def replicate_to_host(
    ...
    # Step 1: Flatten each metric, replicate DTensors, group CUDA tensors by dtype
    ...
    for name, metric in metrics.items():
        ...
        for val in flat_leaves:
            # Replicate DTensors to local tensors
            if isinstance(val, DTensor):
                val = _to_replicated_dtensor(val).to_local()
    ...
    # Step 2: Batch GPU→CPU transfer for each dtype group
    ...
    # Step 3: Reconstruct metrics with scalar leaves, call .value()
    ...
    for name, (spec, leaf_info) in per_metric.items():
        ...
        result[name] = float(val) if isinstance(val, Tensor) else float(val)
