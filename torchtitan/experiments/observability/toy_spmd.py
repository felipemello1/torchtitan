...
class MLPBlock(nn.Module):
    ...
    Scenarios 2+3: per-head child_context inside compile, merged metric outside.
    """
...
class ToyTrainer:
    ...
    def _setup_logging(self, cfg):
        """Set up logging schedule and backends.
    ...
    def train_step(self, tokens, labels, loss_mask, step):
        ...
        # System metrics: always log (every step)
        record_event({"train.loss": loss.item(), "train.grad_norm": grad_norm.item()})

        # Tensor metrics: gated by schedule (expensive all-reduce)
