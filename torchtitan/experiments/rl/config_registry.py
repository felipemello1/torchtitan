...
"""Config entry points for the RL experiment.

Each function returns a complete :class:`RLTrainer.Config` and is
discoverable by ``ConfigManager`` via ``--module rl --config <fn_name>``.

Production surface (in scope for this PR):

- ``rl_grpo_qwen3_0_6b``: SumDigits smoke on Qwen3-0.6B (DAPO loss; name kept
  for backward compatibility with launch scripts).
- ``rl_dapo_qwen3_1_7b_alphabet_sort_2gpu``: fast 2-GPU AlphabetSort gate.
- ``rl_dapo_qwen3_1_7b_alphabet_sort_2gpu_acceptance``: 100-step acceptance.
- ``rl_dapo_qwen3_0_6b_alphabet_sort``: smaller AlphabetSort smoke.
- ``rl_grpo_qwen3_0_6b_batch_invariant``: deterministic on-policy smoke.

4B / 14B configs and the ``_4b_instruct_2507`` model registry entry are
out of scope; ship them in a separate model-registry PR.
"""
...
def _alphabet_sort_dataset(seed: int) -> AlphabetSortDataset.Config:
    ...
    Matches prime-rl's ``rl.toml`` (min_turns=3, max_turns=5) so the per-
    example variance Prime-RL trains on is reproduced here.
    """
...
def _alphabet_sort_builder() -> AlphabetSortBuilder.Config:
    """Shared AlphabetSort scoring parameters (Prime-RL aligned)."""
...
def _dapo_loss() -> DAPOLoss.Config:
    """DAPO clip-higher and dual-clip defaults."""
...
def _alphabet_sort_config(
    *,
    ...
    loss: DAPOLoss.Config | None = None,
) -> RLTrainer.Config:
    ...
    return RLTrainer.Config(
        ...
        trainer=PolicyTrainer.Config(
            ...
            loss=loss if loss is not None else _dapo_loss(),
        ),
...
def _sum_digits_smoke_config(
    *,
    ...
    return RLTrainer.Config(
        ...
        # SumDigits needs the answer in <answer>N</answer>. Qwen3's chat
        # template defaults to thinking mode, which fills 100 tokens with
        # <think>...</think> before the answer. Disable thinking so the
        # 100-token budget reaches the answer tag.
        renderer=RendererConfig(name="qwen3", enable_thinking=False),
        metrics=MetricsProcessor.Config(enable_wandb=True),
        trainer=PolicyTrainer.Config(
            optimizer=OptimizersContainer.Config(lr=lr),
...
def rl_grpo_qwen3_0_6b() -> RLTrainer.Config:
    """SumDigits smoke on Qwen3-0.6B (6 GPUs: 4 gen + 2 train).

    DAPO loss with clip-higher and dual-clip. Name kept for backward
    compatibility with launch scripts.
    """
...
def rl_grpo_qwen3_0_6b_batch_invariant() -> RLTrainer.Config:
    """On-policy SumDigits smoke on Qwen3-0.6B (4 GPUs: 2 gen + 2 train).
