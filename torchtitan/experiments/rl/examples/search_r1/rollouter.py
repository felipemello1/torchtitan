...
# Default Search-R1 NQ/HotpotQA parquet locations (prepared via Search-R1's data_process scripts).
# Placeholders under the example dir — set `train_dataset`/`validation_dataset` data_path in a
# config (or drop the parquets here). See README.md for data + retriever setup.
_DATA_DIR = "torchtitan/experiments/rl/examples/search_r1/data"
DEFAULT_TRAIN_PARQUET = f"{_DATA_DIR}/train.parquet"
DEFAULT_TEST_PARQUET = f"{_DATA_DIR}/test.parquet"
