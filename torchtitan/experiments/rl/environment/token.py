...
class TokenEnv(Configurable):
    ...
    class Config(Configurable.Config):
        ...
        """Hard cap on assistant turns per rollout. When the env would continue past this many
        `step` calls without signaling `done`, the turn is made terminal (`TRUNCATED_LENGTH`).
        `None` disables the check (single-turn envs are unaffected)."""
    ...
    async def step(self, completion: Completion) -> TokenEnvOutput:
        ...
        # Cap the number of turns: if the env wants to continue but we've hit the limit, end the
        # rollout as truncated rather than generating another turn.
        max_turns = self._config.max_num_turns
        if max_turns is not None and self._num_turns >= max_turns:
            return TokenEnvOutput(
                next_prompt_token_ids=None,
                next_prompt_messages=None,
                status=RolloutStatus.TRUNCATED_LENGTH,
