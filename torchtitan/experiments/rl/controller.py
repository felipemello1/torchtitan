...
_batcher_loop
  consumes: FINALIZED groups in finish order (group_buffer.take_finalized)
...
class AsyncLoopConfig(Configurable.Config):
    ...
    """Target steady-state policy age; sets active buffer depth to `(S + 1) * P`.

    Example:
        `S=1, P=4` allows 8 active groups and gives mean age about 1 in steady
        state. An individual straggler may train above age 1 while younger ready
        groups train below it.
    """
...
class Controller(Configurable):
    ...
    async def run(self) -> None:
        ...
        # The active-slot capacity sets the target steady-state policy age.
        max_active_rollout_groups = async_loop.max_active_rollout_groups
        logger.info(
            f"rollout_buffer_capacity={max_active_rollout_groups}, "
        ...
        # One rollout worker per active buffer slot: lets generation fill the whole active-slot budget,
    ...
    async def _data_input_loop(self, group_buffer: RolloutGroupWorkBuffer) -> None:
        ...
        # TODO(perf): Slots are currently released in batches, while this loop is a single producer.
        # we could a) increase the number of threads; b) revisit how we release slots and see if
        # we can release them on the batcher while preserving the end-to-end active-slot budget.
    ...
    async def _rollout_loop(
        ...
        *,
        ...
        The active-slot budget targets mean policy age, but an individual straggler can exceed the target.
        Raw rollouts are recorded before any drop, so dropped groups stay inspectable on disk.
