...
def test_batcher_reports_global_valid_tokens():
    ...
    # Packed: 2 prompt tokens (untrained) + 3 completion tokens (trained); raw len 5 -> 4 after
    # the [:-1]/[1:] shift, trained mask [F,T,T,T] -> 3 response tokens per episode.
    episodes = [
        Episode(
            policy_version=0,
            sample_id=str(i),
            token_ids=[1, 2, 3, 4, 5],
            loss_mask=[False, False, True, True, True],
            logprobs=[0.0] * 5,
            advantage=[0.0, 0.0, float(i), float(i), float(i)],
            version_intervals=[(2, 0)],
        )
        for i in range(4)
    ]
...
def test_batcher_packs_multiple_episodes_into_one_row():
    ...
    # Packed: 1 prompt token + 2 completion tokens; raw len 3 -> 2 after the [:-1]/[1:] shift,
    # trained mask [T,T] -> 2 response tokens per episode.
    episodes = [
        Episode(
            policy_version=0,
            sample_id=str(i),
            ...
            logprobs=[0.0] * 3,
            advantage=[0.0, float(i), float(i)],
            version_intervals=[(1, 0)],
        )
        for i in range(3)
    ]
