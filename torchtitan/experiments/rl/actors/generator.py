...
class VLLMGenerator(Actor, Configurable):
    ...
    async def close(self) -> None:
        ...
        Engine teardown: vLLM's sync `LLMEngine` has no public `shutdown`, so we shut down all components independently.
        1. ``renderer.shutdown()`` — closes thread pools and the multimodal-processor cache.
        2. ``engine_core.shutdown()`` — stops the model worker and the scheduler.

        NOTE: We intentionally skip `cleanup_dist_env_and_memory()`: with `external_launcher`,
        vLLM reuses the process group Monarch created, and destroying it here would prevent
        Monarch's `mesh.stop()` from completing its own collective teardown.
        """
        if self._tp_rank == 0:
            async with self._condition:
