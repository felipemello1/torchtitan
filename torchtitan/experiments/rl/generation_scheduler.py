...
class _PendingGeneration:
    """One controller-side generation request waiting for a batched flush."""
...
class GenerationScheduler:
    """Batch generation requests and pause admission while weights sync.

    Example::

        scheduler = GenerationScheduler(generate_batch, max_admitted_prompts=64)
        task = asyncio.create_task(
            scheduler.submit(
                prompt_token_ids=[1, 2, 3],
                sampling=sampling,
                request_id="step=1/group=0/sample=0",
            )
        )
        await scheduler.pause_for_weight_sync()
        await push_weights()
        await pull_weights()
