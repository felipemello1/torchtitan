...
class TokenEnvConfig:
    """Operational policy for `TokenEnv`.

    Args:
        error_reward: Reward used when parsing or env stepping fails.
        truncation_reward: Reward used for length stops without env reward.
        max_trajectory_tokens: Optional cap before the next generation.
...
@dataclass(kw_only=True, slots=True)
class TokenStep:
    """Result of applying one token completion to a `TokenEnv`."""
...
class TokenEnv:
    ...
    def _raise_if_context_exceeded(self, prompt: PromptState) -> None:
        if self._context_exceeded(prompt):
            ...
            # No rollout turn exists yet, so this setup failure cannot be
            # represented as a terminal env step.
            raise ValueError(
