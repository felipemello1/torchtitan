...
class RolloutGroupWorkBuffer(Configurable):
    ...
    order they finished, whichever group id that is, so a slow group never blocks younger finished ones.
    ...
    class Config(Configurable.Config):
        """No tunables: capacity is derived by the controller."""
    ...
    def __init__(self, config: Config, *, max_active_rollout_groups: int) -> None:
        ...
        # FINALIZED group ids in the order they finished; take_finalized pops from the left.
    ...
    async def finalize_work(self, rollout_group: RolloutGroup) -> None:
        ...
        async with self._condition:
            ...
            self._finalized_group_ids.append(work.group_id)
            self._condition.notify_all()

    @sl.log_trace_span("take_finalized")
    async def take_finalized(self) -> RolloutGroup | None:
        """Batcher loop: return the FINALIZED group that finished first, whichever group id that is.
        ...
        Example:
            ...
            # next call waits until g0 or g1 finalizes; g0 is never skipped, only late.
        """
        async with self._condition:
            while True:
                if self._closed:
                ...
                await self._condition.wait()  # nothing finalized -> generation is the bottleneck
