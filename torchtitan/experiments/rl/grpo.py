...
class RLTrainer(Configurable):
    ...
    def _collate_samples(samples: list[ReplaySample]) -> TrainingBatch:
        ...
        All tensors are packed at full token length ``T``. The trainer
        shifts the loss-side tensors by one (and computes
        ``policy_logprobs`` at the matching ``T-1`` length) after
        microbatch slicing so the slicer can use sample-aligned offsets
        on tensors that all share the same ``T`` dimension.
        ...
        Example::
            ...
            # all four batch tensors are [1, 3]; trainer shifts to [1, 2] later.
        """
        if not samples:
