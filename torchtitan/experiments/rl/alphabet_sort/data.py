...
"""Data generation for AlphabetSort.

Names come from the HF dataset ``kalomaze/alphabetic-arxiv-authors-it1``
(the same source prime-rl's verifiers env uses, ``verifiers/environments/
alphabet_sort/alphabet_sort.py:62``). Loaded once at first access and
flattened/deduped into a single CamelCase pool (~300k+ names); each
``build_example`` deterministically samples its own subset.
"""
...
def _load_names() -> tuple[str, ...]:
    """Load names from the HF dataset once, deduped CamelCase.
...
def build_example(
    *,
    ...
    """Build one deterministic episode from the HF name pool."""
